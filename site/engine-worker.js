...
async function init(req){
  ...
  const decoded=req.name.endsWith('.gz')?new Blob([req.bytes]).stream().pipeThrough(new DecompressionStream('gzip')):new Blob([req.bytes]).stream();
  ...
  M.FS.writeFile('/teacher.bin.gz',new Uint8Array(req.bytes));
  check(await call('kgeLoad',['/teacher.bin.gz',size],['string','number'],true));
...
async function analyze(req){
  ...
  try{
    ...
    tell('complete',{reason:stopReason,actualVisits:frames.at(-1)?.actualRootVisits??0,elapsedMs:Math.round(performance.now()-started),convergence:lastDecision,firstObservedStable:firstStable});
